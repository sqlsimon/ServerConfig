...
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'Application1' AND category_class=1)
BEGIN
EXEC @ReturnCode = msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'Application1'
END


IF EXISTS (SELECT 1 FROM msdb.dbo.sysjobs WHERE name = 'Application1_Job2')
	EXEC msdb.dbo.sp_delete_job @job_name=N'Application1_Job2', @delete_unused_schedule=1


EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Application1_Job2', 
		@enabled=1, 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Application1_Job2', 
		@category_name=N'Application1', 
